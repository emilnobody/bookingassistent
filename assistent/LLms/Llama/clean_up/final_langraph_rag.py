...
#     info.pretty_print()
# print(bustable)
# jsonBustable=json.dumps(bustable)
# # print(jsonBustable)
# python_dict = json.loads(jsonBustable)
# print(type(python_dict))
# print("Json List:", python_dict)

# print(isinstance(bustable, Sequence))
# print(all(isinstance(item, str) for item in bustable))
# print(bustable)
# formated=format_datetime_in_dict(jsonBustable)
# print(formated)
question_template = """now that you have processed and memorized all the information you are able to answer user question below about the following tables {tablenames}
user's question: {question}
"""
...
value=schema_prompt.invoke({"schema": getDBSchema(["buses"])})
nextT=ChatPromptTemplate.from_template(str(value))
user_question_chain=nextT|llm
# user_question_chain=nextT|llm|question_prompt|llm
print(user_question_chain)
# user_question_chain=schema_prompt|question_prompt| llm
# user_question_chain=schema_prompt|llm|question_prompt| llm
llm_response_user_question=user_question_chain.invoke({
        "Gib mir die Anzahl der Busse zurück?",
})
# llm_response_user_question=user_question_chain.invoke({
